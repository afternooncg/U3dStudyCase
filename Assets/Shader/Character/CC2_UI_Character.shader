Shader "CC2/CC2_UI_Character" {
	...
	{       Lighting Off
		...
		inline fixed4 LightingCustomBlinnPhong(SurfaceOutput s, fixed3 lightDir, fixed3 viewDir, fixed atten)
		{
			...
			fixed4 c;
			c.rgb = (s.Albedo * _LightColor0.rgb * diff + _LightColor0.rgb * spec) * (atten * 1.8);
			c.rgb += nv * _RimColor;
		...
		{   
			...
			fixed4 m = tex2D(_FlowMask, IN.uv2_FlowMap);
			fixed4 s = tex2D(_SpecularMap, IN.uv2_FlowMap);
			fixed3 f = tex2D(_FlowMap, IN.uv2_FlowMap + _Time.xx * _FlowSpeed);
			c.rgb = lerp(c.rgb, _MaskColor.rgb, b.b);
			o.Albedo = c.rgb*_Specular;
			o.Gloss = b.rgb * _Emission;
			o.Specular =s.rgb* _Shininess;
			o.Emission = c.rgb * b.rgb * _Emission + f*m.r;
