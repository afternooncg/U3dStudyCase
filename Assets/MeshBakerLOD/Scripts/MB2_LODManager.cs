	...
	public class MB2_LODManager : MonoBehaviour {
		...
		public class BakerPrototype{
			...
			public bool Initialize(BakerPrototype[] bakers){
				...
				if (meshBaker.textureBakeResults == null || meshBaker.textureBakeResults.materials == null){
				...
				for (int j = 0; j < res.materials.Length; j++){
					if (res.materials[j] != null && res.materials[j].shader != null){
						materials.Add(res.materials[j]);
					} else {
