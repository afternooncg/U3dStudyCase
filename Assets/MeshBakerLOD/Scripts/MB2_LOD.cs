...
//		if ((Time.frameCount + framesBetweenLODChangedChecksOffset) % framesBetweenLODChangedChecks != 0) return;
		...
		if (lodIdx != nextLODidx){//if LOD needs to change
			...
			if (combinedMesh.GetClusterManager().GetBakerPrototype().clusterType == MB2_LODManager.BakerPrototype.CombinerType.grid && bakeIntoCombined && _position != transform.position){
